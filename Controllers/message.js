      ...
      if (result.length > 0) {
        ...
        await Message.update({
          conversationId: result[0]._id
            ...
            res.status(httpStatus.OK).json({ message: 'Message added successfully' });
          }).catch(err => {
            res.status(httpStatus.INTERNAL_SERVER_ERROR).json({ message: 'Error while adding message!' });
          });
      } else {
        ...
        await newMessage.save().then(() => {
          ...
          res.status(httpStatus.INTERNAL_SERVER_ERROR).json({ message: 'Error while sending message!' });
        });
      }
    });
  }
